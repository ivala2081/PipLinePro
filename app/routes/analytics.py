...
@handle_errors
def dashboard():
    """Dashboard page with key metrics - PERFORMANCE OPTIMIZED"""
    try:
        # Get date range (default to last 30 days)
        days = request.args.get('days', 30, type=int)
        
        # Use performance optimized query service
        # dashboard_data = performance_optimized_service.get_dashboard_data_optimized(days)
        
        # Temporary fallback - create basic dashboard data
        dashboard_data = {
            'current_period': {
                'total_amount': 0,
                'total_commission': 0,
        ...
        db_health = 95  # Placeholder
        memory_health = 85  # Placeholder
        cpu_health = 90  # Placeholder
        session_health = 100  # Placeholder
        system_uptime = 99.9  # Placeholder
        response_time = 0.15  # Placeholder
        
        now = datetime.now()
        period_label = f"Last {days} days"
        
        # Extract data from optimized service - ensure all values are serializable
        current = dashboard_data.get('current_period', {})
        previous = dashboard_data.get('previous_period', {})
