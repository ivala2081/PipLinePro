...
def get_current_rate():
    ...
    """
    try:
        current_rate = exchange_rate_service.get_current_rate()
        ...
        is_stale = current_rate.is_stale(max_age_minutes=15)
        
        return jsonify({
            'success': True,
            'rate': current_rate.to_dict(),
            'is_stale': is_stale,
            'age_minutes': current_rate.age_in_minutes(),
            'message': 'Stale rate - update recommended' if is_stale else 'Current rate'
        })
    ...
    try:
        ...
        if success:
            ...
            return jsonify({
                ...
                'rate': current_rate.to_dict() if current_rate else None
            })
        else:
            return jsonify({
                'success': False,
    ...
    try:
        ...
        if from_currency == 'USD' and to_currency == 'TRY':
            if custom_rate:
                ...
                converted_amount = exchange_rate_service.convert_usd_to_try(amount_decimal)
                current_rate = exchange_rate_service.get_current_rate()
                rate = current_rate.rate if current_rate else Decimal('27.5')
                rate_source = current_rate.source if current_rate else 'fallback'
